...
// Initialize LLMManager lazily to ensure environment variables are available at runtime
let llmManager: any = null;
let PROVIDER_LIST: any = null;
let DEFAULT_PROVIDER: any = null;
...
function getLLMManager() {
  if (!llmManager) {
    llmManager = LLMManager.getInstance(import.meta.env);
  }

  return llmManager;
}

function getProviderList(): BaseProvider[] {
  if (!PROVIDER_LIST) {
    PROVIDER_LIST = getLLMManager().getAllProviders();
  }

  return PROVIDER_LIST;
}

function getDefaultProvider(): BaseProvider {
  if (!DEFAULT_PROVIDER) {
    DEFAULT_PROVIDER = getLLMManager().getDefaultProvider();
  }

  return DEFAULT_PROVIDER;
}

// Export functions instead of direct values
export { getProviderList as PROVIDER_LIST, getDefaultProvider as DEFAULT_PROVIDER };

export const providerBaseUrlEnvKeys: Record<string, { baseUrlKey?: string; apiTokenKey?: string }> = {};
getProviderList().forEach((provider: any) => {
